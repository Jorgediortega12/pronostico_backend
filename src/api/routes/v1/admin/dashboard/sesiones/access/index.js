...
export const cargarDatosSesiones = async (req, res) => {
    try {
        ...
        if (!codsuperior) {
            return responseError(200, 'Parametro codsuperior no proporcionado', 400, res);
        }
...
export const cargarArchivosVrSesiones = async (req, res) => {
    try {
        ...
        if (!codcarpeta) {
            return responseError(200, 'Parametro codcarpeta no proporcionado', 400, res);
        }
...
export const cargarPeriodosSesion = async (req, res) => {
    try {
        ...
        if (!codsesion || !tipo) {
            return responseError(200, 'Parametro codsesion y tipo no proporcionado', 400, res);
        }
...
export const cargarPeriodosxUCPxFecha = async (req, res) => {
    try {
        ...
        if (!ucp || !fecha) {
            return responseError(200, 'Parametros ucp y fecha no proporcionados', 400, res);
        }
