...
async function main() {
  ...
  try {
    ...
    if (check.rows.length > 0) {
      ...
      console.log(`El módulo '/analisis_datos' ya existe con cod=${codMenu}. Saltando inserción.`);
    } else {
    ...
    for (const perfil of perfiles.rows) {
      ...
      if (yaAsignado.rows.length > 0) {
        console.log(`  → Perfil "${perfil.nombre}" (${perfil.cod}): ya tenía el módulo asignado.`);
      } else {
        await client.query(
          "INSERT INTO usu_usuarioacceso (codperfil, codmenu) VALUES ($1, $2)",
          [perfil.cod, codMenu]
        );
        console.log(`  ✓ Perfil "${perfil.nombre}" (${perfil.cod}): módulo asignado.`);
      }
    }
  } finally {
    await client.end();
