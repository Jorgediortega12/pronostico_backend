...
if (!MODEL_ID || isNaN(MODEL_ID)) {
  console.error("❌ Debes pasar el model_id como argumento. Ej: node scripts/migrateModelValues.js 262");
  process.exit(1);
}
...
async function migrate() {
  ...
  try {
    ...
    if (!gmrRows.length) {
      console.log(`⚠️  No se encontraron valores para model_id=${MODEL_ID} en GMR.`);
      return;
    }
    ...
    if (deleted > 0) {
      console.log(`🗑️  Se eliminaron ${deleted} filas previas del modelo ${MODEL_ID} en local.`);
    }
    ...
    console.log(`✅ Migración completada: ${gmrRows.length} filas insertadas para model_id=${MODEL_ID}.`);
    ...
    console.log(`🔍 Verificación: ${verify[0].total} filas en DB local para model_id=${MODEL_ID}.`);
