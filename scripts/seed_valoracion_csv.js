...
async function main() {
  ...
  const ESCENARIOS = [
    ...
    { src: "RESTRICCIONES_ADICIONALES", dest: "restricciones_adicionales", map: toText },
    ...
    { src: "GENERACIONES_ESTANCAMIENTO", dest: "generaciones_estancamiento", map: toInt },
  ];
  ...
  const RESULTADOS = [
    ...
    { src: "PORCENTAJES_CONTRATOS", dest: "porcentajes_contratos", map: toText },
    { src: "PORCENTAJES_POR_PERIODO", dest: "porcentajes_por_periodo", map: toText },
    { src: "ENERGIA_TOTAL_COMPRADA", dest: "energia_total_comprada", map: toFloat },
    { src: "COBERTURA_PORCENTUAL", dest: "cobertura_porcentual", map: toFloat },
    { src: "ERROR_CUADRATICO_MEDIO", dest: "error_cuadratico_medio", map: toFloat },
    ...
    { src: "ENERGIA_POR_CONTRATO_PERIODO", dest: "energia_por_contrato_periodo", map: toText },
    { src: "COSTO_POR_CONTRATO_PERIODO", dest: "costo_por_contrato_periodo", map: toText },
    { src: "TIPOS_CONTRATOS_UTILIZADOS", dest: "tipos_contratos_utilizados", map: toText },
  ];
  ...
  try {
    ...
    const nOf = await insertar(client, "SphaerAI_ofertas_valoracion", OFERTAS, ofertas);
    console.log(`  ✓ Ofertas insertadas:    ${nOf}/${ofertas.length}`);

    const nEs = await insertar(client, "SphaerAI_escenarios_valoracion", ESCENARIOS, escenarios);
    console.log(`  ✓ Escenarios insertados: ${nEs}/${escenarios.length}`);

    const nRe = await insertar(client, "SphaerAI_resultados_optimizacion", RESULTADOS, resultados);
